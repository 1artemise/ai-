...
def generate_result(predicted_imps_lst, predicted_fins_lst,args):
    pre_imp_dict = {}
    pre_imp_fin_dict = {}
    if torch.cuda.is_available():
        predicted_imps_lst = predicted_imps_lst.cpu().data.numpy()
        predicted_fins_lst = predicted_fins_lst.cpu().data.numpy()
    else:
        predicted_imps_lst = predicted_imps_lst.data.numpy()
        predicted_fins_lst = predicted_fins_lst.data.numpy()
    with open(args.vocab_path, 'rb') as f:
        vocab = pickle.load(f)
    assert len(predicted_imps_lst) == len(predicted_fins_lst)

    # print(predicted_imps_lst.shape[0])
    # print(predicted_fins_lst.shape[0])

    # Impressions:
    pre_imp = _imp_generator(predicted_imps_lst, 0, vocab)

    # Findings:
    pre_fin = _fin_generator(predicted_fins_lst, 0, vocab)


    return pre_imp,pre_fin
