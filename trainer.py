...
import argparse
import torch
import torchvision.transforms as trpiansforms
...
def train_net(num_run, logger, args):
    ...
    for epoch in range(args.epochs):
        ...
        if max(imp_fin_bleu4_lst[:-1]) < imp_fin_bleu[3]:
            ...
            torch.save(best_img_encoder, os.path.join(
                'try_pth/', '{}-image_encoder-{}.pth'.format(num_run + 1, best_epoch + 1)))
            torch.save(best_imp_decoder, os.path.join(
                'try_pth/', '{}-impression_decoder-{}.ckpt'.format(num_run + 1, best_epoch + 1)))
            torch.save(best_fin_decoder, os.path.join(
                'try_pth/', '{}-finding_decoder-{}.ckpt'.format(num_run + 1, best_epoch + 1)))
        scheduler_imp.step()
        scheduler_fin.step()
    ...
    torch.save(best_img_encoder, os.path.join(
        'result_pth/', '{}-image_encoder-{}.pth'.format(num_run + 1, best_epoch + 1)))
    torch.save(best_imp_decoder, os.path.join(
        'result_pth/', '{}-impression_decoder-{}.pth'.format(num_run + 1, best_epoch + 1)))
    torch.save(best_fin_decoder, os.path.join(
        'result_pth/', '{}-finding_decoder-{}.pth'.format(num_run + 1, best_epoch + 1)))
